...
const ToolsSoldiers = () => {
  ...
  const Tools = rows?.filter(row => row.departmentName === 'ادوات') ?? [];
  ...
  return (
    <div>
      <header>
        <HeaderTop NavTitle="اطلاعات سربازان ادوات" />
        <Menu />
      </header>
      <main>
        <div className={"h-full max-[768px]:mr-0" + (showMenu ? " mr-64" : " mr-[4.25rem]")}>
          <div className="py-4 px-4">
            <div className="container px-4">
              <HeaderBottom titr="سرباز ادوات" redirect={"/AmadSoldier"} />
            </div>
            <Table data={Tools}/>
          </div>
        </div>
