...
const AmadSoldiers = () => {
    ...
    const Amad = rows?.filter(row => row.departmentName === 'آماد') ?? [];
    ...
    return (
      <div>
        <header>
          <HeaderTop NavTitle='اطلاعات سربازان آماد' />
          <Menu />
        </header>
        <main>
          <div className={'h-full max-[768px]:mr-0' + (showMenu ? " mr-64" : " mr-[4.25rem]")}>
            <div className='py-4 px-4'>
              <div className='container px-4'>
                <HeaderBottom titr='سرباز آماد' redirect={'/AmadSoldier'} />
              </div>
              <Table data={Amad}/>
            </div>
          </div>
