...
const CampSoldiers = () => {
  ...
  const Camp = rows?.filter(row => row.departmentName === 'قرارگاه') ?? [];
  ...
  return (
    <div>
      <header>
        <HeaderTop NavTitle="اطلاعات سربازان قرارگاه" />
        <Menu />
      </header>
      <main>
        <div className={"h-full max-[768px]:mr-0" + (showMenu ? " mr-64" : " mr-[4.25rem]")}>
          <div className="py-4 px-4">
            <div className="container px-4">
              <HeaderBottom titr="سرباز قرارگاه" redirect={"/AmadSoldier"} />
            </div>
            <Table data={Camp}/>
          </div>
        </div>
